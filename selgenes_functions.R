...
selgenes_method_SFCS <- function(data_pack, xgene, method = "pvalue", sd_factor = 2, parallelized = T, pvalue = 0.05) {
  ...
  if (parallelized) {
    ...
    score <- foreach(col=1:ncol(datat), .combine=rbind) %do% {
      if (col %% step == 0)
        incProgress(0.1, detail = paste0("(Gene ", col, ")"))
      metric_SFCS(datat[,xgene,with=F], datat[,col,with=F])
    }
  }
...
readOutputs <- function (geneName, geneTypes = "protein_coding", baseFolder = "output_genes") {
  ...
  res$direct = fread(paste0(baseFolder, "/", geneName, "_selgenes_direct.csv"))
  res$direct_scores = fread(paste0(baseFolder, "/", geneName, "_selgenes_direct_scores.csv"))
  res$inverse = fread(paste0(baseFolder, "/", geneName, "_selgenes_inverse.csv"))
  res$inverse_scores = fread(paste0(baseFolder, "/", geneName, "_selgenes_inverse_scores.csv"))
